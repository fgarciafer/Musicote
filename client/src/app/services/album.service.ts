...
export class AlbumService{
    ...
    getAlbumsPerPage(token,page){
        ...
        let options = new RequestOptions({headers:headers});
        return this._http.get(this.url+'allAlbums/'+page,options).map(
            res => res.json());
    }


    getAlbumsByName(token, search,page){
        let headers = new Headers({
            'Content-Type':'application/json',
            'Authorization':token
        });
        ...
        let options = new RequestOptions({headers:headers});
        return this._http.get(this.url+'albumsName/'+search+'/'+page,options).map(res => res.json());
    }
